...
### For blackbox: ´ set debug_mode = DYN_LPF2 ª
...
Main parameters are avalable in OSD : ´ Filter GLB ª => ´ Params starting with DYNLPF2 ª.

-	´ DLPF2 ENABLE ª: 0 to disable filter / 1 to enable.

-	´ DLPF2 THRO_BRKPT ª : is a throttle threshold in %. When throttle is above this value, Fmin will rise according to DYNLP2 THRO_GAIN.

-	´ DLPF2 THRO_GAIN ª : is in Hz / % throttle. It is how fast Fmin rise when throttle is above ´ DYNLP2 THRO_BRKPT ª. Example : if ´ DYNLP2 THRO_BRKPT ª=38%  and  ´ DYNLP2 THRO_GAIN ª  = 10Hz/% => When throttle = 50%. Fmin = 60Hz + (50-38)*10 => 180Hz.You can tune DYNLP2 THRO_BRKPT  / DYNLP2 THRO_BRKPT to found the good spot between smooth when throttle is over 38% vs propwash.

-	´ DLPF2 GAIN ª : is to increase/decrese sensitivity according to error.if value is big (100), cutoff freq of the filter will rise really quick on gyro ñ setpoint error.Lower this value will make it smoother, but also more sensitive to propwash. (Start with default).

-	´ DLPF2 FMINª : is the min/base frequency of the filter.60Hz seems to be good on 5íí
...
We want to make Betaflight accessible for pilots who are not fluent in English, and for this reason we are currently maintaining translations into 18 languages for Betaflight Configurator: Catal√†, Deutsch, Espa√±ol, Euskera, Fran√ßais, Galego, Hrvatski, Bahasa Indonesia, Italiano, Êó•Êú¨Ë™û, ÌïúÍµ≠Ïñ¥, Latvie≈°u, Portugu√™s, Portugu√™s Brasileiro, polski, –†—É—ù—ù–∫–∏–π —ù–∑—ã–∫, Svenska, ÁÆÄ‰Ωì‰∏≠Êñá.
