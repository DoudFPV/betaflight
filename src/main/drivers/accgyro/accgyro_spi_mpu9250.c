...
void mpu9250SpiGyroInit(gyroDev_t *gyro)
{
    ...
    spiSetClkDivisor(&gyro->dev, SPI_CLOCK_FAST); //high speed now that we don't need to write to the slow registers
...
static void mpu9250AccAndGyroInit(gyroDev_t *gyro) {
    ...
    spiSetClkDivisor(&gyro->dev, SPI_CLOCK_INITIALIZATION); //low speed for writing to slow registers
