...
static bool flashSpiInit(const flashConfig_t *flashConfig)
{
     ...
     */
    static uint8_t readIdResponse[4] = { 0 };
...
void flashPageProgramContinue(const uint8_t **buffers, uint32_t *bufferSizes, uint32_t bufferCount)
{
    flashDevice.vTable->pageProgramContinue(&flashDevice, buffers, bufferSizes, bufferCount);
}
