...
static void w25n01g_programDataLoad(flashDevice_t *fdevice, uint16_t columnAddress, const uint8_t *data, int length)
{
    ...
    if (fdevice->io.mode == FLASHIO_SPI) {
        ...
        uint8_t cmd[] = { W25N01G_INSTRUCTION_PROGRAM_DATA_LOAD, columnAddress >> 8, columnAddress & 0xff };
         ...
         spiSequence(dev, &segments[0], true);
...
void w25n01g_pageProgramContinue(flashDevice_t *fdevice, uint8_t const **buffers, uint32_t *bufferSizes, uint32_t bufferCount)
{
    if (bufferCount < 1) {
        fdevice->callback(0);
        return;
    ...
    // This code is non-optimal in that it only writes the first buffer, subsequent writes requiring a fresh flush
