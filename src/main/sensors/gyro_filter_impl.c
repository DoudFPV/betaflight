...
static FAST_CODE void GYRO_FILTER_FUNCTION_NAME(void)
{
    ...
    //Test filters at 10hz
    float input = gyro.gyroADCf[X];
    float biquad = biquadFilterApply(&TestBiquad, input);
    float biquadImuf = biquad_imuf_update(input, &TestBiquadImuf);

    //Log
    DEBUG_SET(DEBUG_ALTITUDE, 0, gyro.gyroADCf[X]);
    DEBUG_SET(DEBUG_ALTITUDE, 1, biquad);
    DEBUG_SET(DEBUG_ALTITUDE, 2, biquadImuf);
